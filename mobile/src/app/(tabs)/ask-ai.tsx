...
const examples = [
  ...
  'What medication doses were recorded today?',
];
...
function EvidenceModal({ evidence, onClose }: { evidence: AskEvidence[] | null; onClose: () => void }) {
  ...
  const openRecord = (item: AskEvidence) => {
    if (item.record_type !== 'medication' && item.record_type !== 'medication_dose') return;
    onClose();
    router.push({ pathname: '/medications', params: { evidenceType: item.record_type, recordId: item.record_id } });
  };
  return <Modal visible={evidence !== null} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}><SafeAreaView style={styles.safe} edges={['top', 'bottom']}><View style={styles.modalHeader}><View><Text style={styles.brand}>Ask CareRelay</Text><Text style={styles.modalTitle}>Supporting records</Text></View><Pressable accessibilityRole="button" accessibilityLabel="Close evidence" onPress={onClose} style={styles.close}><Ionicons name="close" size={24} color={colors.ink} /></Pressable></View><ScrollView contentContainerStyle={styles.evidenceList}>{evidence?.map((item) => { const navigable = item.record_type === 'medication' || item.record_type === 'medication_dose'; return <Pressable key={`${item.record_type}-${item.record_id}`} accessibilityRole={navigable ? 'button' : undefined} onPress={navigable ? () => openRecord(item) : undefined} style={styles.evidenceCard}><Text style={styles.evidenceKind}>{evidenceKindLabel(item.record_type)}</Text><Text style={styles.evidenceLabel}>{item.label}</Text><Text style={styles.evidenceTime}>{formatCheckinTime(item.occurred_at)}</Text>{navigable ? <Text style={styles.openRecord}>Open exact record →</Text> : null}</Pressable>; })}</ScrollView></SafeAreaView></Modal>;
}
...
const styles = StyleSheet.create({
  ...
  openRecord: { color: colors.primary, fontSize: 13, fontWeight: '800', marginTop: 10 },
});
